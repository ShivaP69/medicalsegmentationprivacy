...
"""Preview
NestedUNet (UNet++) model for COVID-19 Lung Infection Segmentation from CT Scans
...
class NestedUNet_GroupNorm(nn.Module):
    """
    NestedUNet (UNet++) architecture with GroupNorm for COVID-19 binary segmentation
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.conv0_0 = VGGBlock(
            input_channels, nb_filter[0], nb_filter[0], num_groups=num_groups
        )
        self.conv1_0 = VGGBlock(
            nb_filter[0], nb_filter[1], nb_filter[1], num_groups=num_groups
        )
        self.conv2_0 = VGGBlock(
            nb_filter[1], nb_filter[2], nb_filter[2], num_groups=num_groups
        )
        self.conv3_0 = VGGBlock(
            nb_filter[2], nb_filter[3], nb_filter[3], num_groups=num_groups
        )
        self.conv4_0 = VGGBlock(
            nb_filter[3], nb_filter[4], nb_filter[4], num_groups=num_groups
        )
