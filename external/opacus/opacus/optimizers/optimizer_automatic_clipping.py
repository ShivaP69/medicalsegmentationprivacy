...
class DPAutomaticClippingOptimizer(DPOptimizer):
    ...
    *"Automatic Clipping: Differentially Private Deep Learning Made Easier and Stronger"*
    ...
    def clip_and_accumulate(self):
        ...
        valid_grad_samples = [
            g for g in self.grad_samples
            if g is not None and g.numel() > 0 and len(g) > 0
        ]

        if len(valid_grad_samples) == 0:
            # nothing to accumulate for this step
            return

        per_param_norms: List[torch.Tensor] = [
            g.reshape(len(g), -1).norm(2, dim=-1) for g in valid_grad_samples
        ]
