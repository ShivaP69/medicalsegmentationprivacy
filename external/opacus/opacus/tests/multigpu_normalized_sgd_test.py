...
from opacus.optimizers.ddp_normalized_sgd_optimizer import (
...
def demo_basic(rank, weight, world_size, dp):
    ...
    if dp:
        ddp_model, optimizer, criterion, data_loader = privacy_engine.make_private(
            ...
            noise_multiplier=0,
            max_grad_norm=regularization_param,
            poisson_sampling=False,
            grad_sample_mode="hooks",
            clipping="normalized",
            distributed=True,
        )
        assert isinstance(optimizer, DistributedNormalizedSGDPOptimizer)
...
class GradientComputationTestNormalizedSGD(unittest.TestCase):
    @unittest.skipIf(torch.cuda.device_count() < 2, "Need at least 2 GPUs")
    def test_normalized_sgd_gradient_correct(self) -> None:
